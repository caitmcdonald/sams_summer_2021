...
# Dataset: endogenous/exogenous FeLV in vitro gene expression (fibroblasts and PMBCs, uninfected only)

# This code will do the following:
# 1. Generate filtered and normalized DGElists
...
#start with counts and metadata, with outliers removed: files previously generated by sam_DGE_dataexplore.R
#load data for each tissue separately and run code

##counts and metadata
counts <- read_delim("results/edgeR/counts_uninf.txt", delim="\t")
metadat <- read_delim("results/edgeR/meta_uninf.txt", delim="\t")

group <- metadat$cell_type
...
pca <- ggplot(data=scores, aes(x=PC1, y=PC2, group=group)) + 
  ...
  xlab ("PC1 (84.8%)") +
  ylab ("PC2 (4.9%)") +
  guides(colour = guide_legend(override.aes = list(size=2), title="Cell type")) 
pca + ggrepel::geom_text_repel(aes(label=metadat$cat_id))
# ggsave(file="plots/pca_cell_type.jpg")
